...
@section('page')
  ...
  k
                ...
                <div class="bg-white border border-gray-200 rounded-3xl shadow-sm 2xl:col-span-2 dark:border-gray-700 sm:p-6 dark:bg-blek-700">
                    <div class="2xl:col-span-2 sm:p-6 ">
                        ...
                        <div class="flex items-center p-2.5 mb-1 mt-2 w-[40%] text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                            ...
                            <div>
                              <span class="font-medium">Perhatian!</span> Terdapat {{ $data->belumcount }} jadwal belum dibuat
                            </div>
                          </div>
                        @endif
                    </div>
                                ...
                                <form class="p-4 md:p-5" action = "buatjadwal/{{ $jadwal->id }}" method = "POST">
                                    ...
                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                        ...
                                        <div class = "col-span-1 mr-8">
                                            <button type="submit" class="text-white w-full inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                                <svg class="me-1 -ms-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                                                Buat Jadwal
                                            </button>
                                        </div>
                                        <div class = "col-span-1 ml-8">
                                            <button type="button" class="random-generate w-full text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center " onclick="generateRandomSchedule('{{ $jadwal->id }}')">
                                                rrandomize..
                                            </button>
                                        </div>
        ...
        <script>
            ...
            function generateRandomSchedule(id) {
                // Generate random day (1-5)
                const randomDay = Math.floor(Math.random() * 5) + 1;
                
                // Get available rooms from the select options
                const roomOptions = Array.from($(`#ruang-${id} option`))
                    .filter(option => option.value !== "Ruang")
                    .map(option => option.value);
                const randomRoom = roomOptions[Math.floor(Math.random() * roomOptions.length)];
                
                // Available time slots (0-11, excluding 5 and 8 as per your original options)
                const availableTimeSlots = [0, 1, 2, 3, 4, 6, 7, 9, 10, 11];
                const randomTimeSlot = availableTimeSlots[Math.floor(Math.random() * availableTimeSlots.length)];
                
                // Set the values
                $(`#hari-${id}`).val(randomDay);
                $(`#ruang-${id}`).val(randomRoom);
                $(`#jam-${id}`).val(randomTimeSlot);

                //set values of jamselesai
                let sks = parseInt($(`#sks-${id}`).data('sks'));
                let jamSelesai = jamEnd[randomTimeSlot + sks];
                $(`#jamselesai-${id}`).val(jamSelesai);
                ...
                $(`#hari-${id}, #ruang-${id}, #jam-${id}`).trigger('change');
            }
